...
const SkillCard = ({ title, dataDesc, icon: Icon }: { title: string, dataDesc: React.ReactNode, icon: any }) => (
  <motion.div whileHover={{ y: -5 }} className="bg-white p-6 rounded-2xl shadow-sm border border-ink/5 hover:shadow-xl transition-all flex flex-col relative overflow-hidden h-full group">
    ...
    <div className="text-[13px] text-ink/60 leading-relaxed font-medium relative z-10">
      {dataDesc}
    </div>
  </motion.div>
);
  ...
  return (
    <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="relative pl-8 mb-12 group last:mb-0">
      ...
      <div className="flex flex-wrap items-center gap-4 mb-2">
        <h4 className="text-lg font-black tracking-tight text-ink">
      ...
      <p className="text-[13px] text-ink/60 leading-relaxed max-w-xl font-medium">{desc}</p>
      {details && (
        <AnimatePresence>
          {isExpanded && (
            <motion.div initial={{ height: 0, opacity: 0, marginTop: 0 }} animate={{ height: 'auto', opacity: 1, marginTop: '1.5rem' }} exit={{ height: 0, opacity: 0, marginTop: 0 }} className="overflow-hidden">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pb-4">
                <div className="bg-white p-5 rounded-2xl border border-ink/5">
                  ...
                  <ul className="space-y-2.5">{details.content?.map((item: string, i: number) => (<li key={i} className="text-[12px] text-ink/70 relative pl-3"><span className="absolute left-0 top-1.5 w-1 h-1 bg-ink/20 rounded-full" />{item}</li>))}</ul>
                </div>
                <div className="bg-rust/[0.03] p-5 rounded-2xl border border-rust/10">
                  ...
                  <ul className="space-y-2.5">{details.projects?.map((item: string, i: number) => (<li key={i} className="text-[12px] text-rust/80 relative pl-3 font-medium"><span className="absolute left-0 top-1.5 w-1 h-1 bg-rust/40 rounded-full" />{item}</li>))}</ul>
                </div>
                <div className="bg-rust p-5 rounded-2xl shadow-lg">
                  ...
                  <ul className="space-y-2.5">{details.results?.map((item: string, i: number) => (<li key={i} className="text-[12px] text-white relative pl-3 font-medium"><span className="absolute left-0 top-1.5 w-1 h-1 bg-white/50 rounded-full" />{item}</li>))}</ul>
                </div>
              </div>
            </motion.div>
          )}
      ...
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="relative w-full max-w-2xl rounded-[2rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh] bg-white">
          ...
          <div className="p-8 overflow-y-auto">
            ...
            <p className="text-[14px] text-ink/70 leading-relaxed font-medium mb-8">{project.desc}</p>
            <div className="bg-[#F8F9FB] p-6 rounded-2xl border border-ink/5">
              ...
              <p className="text-[14px] text-ink/80 leading-relaxed">{project.detail}</p>
            </div>
          </div>
        </motion.div>
      </div>
...
const ProjectCard = ({ project, onClick }: { project: any; onClick: () => void }) => (
  <motion.div onClick={onClick} whileHover={{ y: -8 }} className="relative rounded-[2rem] flex flex-col cursor-pointer shadow-sm border border-ink/5 bg-white overflow-hidden h-[300px] group transition-all hover:shadow-xl">
    <div className="relative h-[160px] w-full overflow-hidden bg-ink/5">
      ...
      <div className="absolute top-4 left-4">
        <span className="text-white text-[10px] font-bold uppercase tracking-wider bg-black/40 backdrop-blur-md px-3 py-1 rounded-full border border-white/20 flex items-center gap-1.5 shadow-sm">
           {project.tag}
        </span>
      </div>
    </div>
    <div className="p-6 pt-4 flex flex-col flex-1 bg-white">
      <h3 className="text-lg font-black mb-2 text-ink leading-tight tracking-tight">{project.title}</h3>
      <p className="text-[13px] text-ink/50 leading-relaxed font-medium line-clamp-2">{project.desc}</p>
      <div className="mt-auto">
        <span className="text-rust font-bold text-[12px] tracking-widest flex items-center gap-1 group-hover:gap-3 transition-all">了解详情 &rarr;</span>
      </div>
    </div>
  </motion.div>
);
...
const AILabCard = ({ title, tag, desc, bgColor, mockup }: { title: string; tag: string; desc: string; bgColor: string; mockup: React.ReactNode }) => (
  <motion.div whileHover={{ y: -5 }} className={`${bgColor} rounded-[2.5rem] p-8 flex flex-col min-h-[200px] relative group border border-ink/5 mt-10 shadow-sm hover:shadow-md transition-all`}>
    <div className="absolute -top-12 -right-4 w-32 h-40 z-20 pointer-events-none group-hover:scale-105 transition-transform">{mockup}</div>
    <div className="z-10 relative max-w-[65%]">
      <span className="text-[10px] font-black uppercase tracking-widest text-ink/50 bg-white/60 backdrop-blur-md px-3 py-1.5 rounded-full border border-white/60 mb-4 inline-block">{tag}</span>
      <h3 className="text-xl font-black mb-3 text-ink leading-tight tracking-tight">{title}</h3>
      <p className="text-[13px] text-ink/60 leading-relaxed font-medium line-clamp-3">{desc}</p>
    </div>
  </motion.div>
);
...
const VideoMockup = ({ src, rotateClass = "rotate-3" }: { src: string, rotateClass?: string }) => {
  ...
  return (
    <div className={`w-full h-full rounded-2xl overflow-hidden shadow-2xl border-[4px] border-white/90 bg-white ${rotateClass} transition-transform`}>
...
const FULL_FALLBACK = {
  skills: [
    ...
    { title: "复合运营背景与商业思维", desc: <>能够围绕平台流量、供给与用户特征，<strong className="text-ink font-bold">输出内容营销策略和行业解决方案</strong>。</> }
  ],
  timeline: [
    { type: 'Work', date: "2025.10 - 至今", title: "餐厅合伙人 / 企业顾问", company: "予童科技等", desc: "主导BP撰写与融资对接，负责婴幼儿家庭AI服务产品孵化与培训体系搭建。" },
    { type: 'Work', date: "2021.07 - 2025.07", title: "运营经理", company: "Ctrip 携程", desc: "从0-1搭建商家直播生态体系，3年推动平台直播GMV从1000万增至10亿+。", details: { content: ["从 0 到 1 搭建商家直播生态体系","制定直播间运营标准与流量分发策略"], projects: ["携程直播青训营","携程 AI 虚拟人直播"], results: ["3 年推动 GMV 增至 10 亿+","转化率提升 70%+"] } },
    { type: 'Work', date: "2020.05 - 2021.07", title: "产品运营", company: "Yitiao 一条", desc: "0-1艺术电商平台搭建，优化用户注册转化节点，将小程序注册率提升至80%。" },
    { type: 'Education', date: "2024.09 - 2027.03", title: "工商管理 (MBA)", company: "复旦大学 (在读)", desc: "专注于商业领导力与创新管理，参与 Esade University 交换项目。" }
  ],
  projects: [
      ...
      <main className="max-w-5xl mx-auto px-6 pt-40 pb-20">
        ...
        <footer className="pt-20 pb-12 border-t border-ink/10 flex flex-col items-center">
          <div className="w-full flex flex-col md:flex-row justify-between items-center gap-8">
            ...
            <div className="flex gap-4">
              <button onClick={() => setIsWeChatOpen(true)} className="bg-ink text-white px-8 py-3 rounded-full text-xs font-bold shadow-lg">微信</button>
              <a href="mailto:zwy0025@gmail.com" className="bg-white border border-ink/10 px-8 py-3 rounded-full text-xs font-bold">邮件</a>
            </div>
          </div>
        </footer>
      </main>
